...
public class MediaFocusControl implements OnFinished {
    ...
    private boolean canReassignAudioFocus() {
		...
		/*[liuqing][modify]modify to make can play the music when in a call*/
            return true;//false;
        }
        return true;
    }
    ...
    private void filterMediaKeyEvent(KeyEvent keyEvent, boolean needWakeLock) {
        ...
        synchronized(mRingingLock) {
            synchronized(mRCStack) {
                /// M: treat MODE_IN_CALL, MODE_IN_CALL_2, MODE_IN_CALL_EXTERNAL as incall mode @{
                if ((mMediaReceiverForCalls != null)
                        && (mIsRinging || (mAudioService.getMode() == AudioSystem.MODE_IN_CALL)
                                || (mAudioService.getMode() == AudioSystem.MODE_IN_CALL_2) || (mAudioService
                                .getMode() == AudioSystem.MODE_IN_CALL_EXTERNAL))) {
